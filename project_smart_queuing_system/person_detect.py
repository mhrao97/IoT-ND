...
class PersonDetect:
    ...
    def predict(self, image):
        ...
        req_handle = self.net.start_async(request_id=0, inputs=input_dict)
        req_status = req_handle.wait()
        if req_status == 0:
            outputs = req_handle.outputs[self.output_name]
    ...
    def draw_outputs(self, coords, image, width, height):
        ...
        return image, people_count, people_frame
...
def main(args):
    ...
    try:
        while cap.isOpened():
            ...
            frame, ppl_count, coords = pd.draw_outputs(coords, image, initial_w, initial_h)
